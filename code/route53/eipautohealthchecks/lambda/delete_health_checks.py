...
def delete_health_checks(cfnStackPrefix):
    ...
    logger.debug("DELETE TIME: " + cfnStackName)
    waitForCFN = True
    timeKeeper = 0
    cfnAction = "evaluate"
    while waitForCFN:
        ...
        try:
            ...
            if error.response['Error']['Message'] == "Stack with id " + cfnStackName + " does not exist":
                return ()
        tagFound = False
        for tags in cfn_response['Tags']:
            if tags['Key'] == 'FunctionName' and tags['Value'] == functionName:
                tagFound = True
        if not tagFound:
            logger.error("Stack did not have required Tag")
            return()
